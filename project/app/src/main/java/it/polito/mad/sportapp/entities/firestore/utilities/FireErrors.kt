...
/**
 * Generic error Type for a failed generic Firestore operation. It just
 * returns a message() string explaining the reason of the error
 */
interface FireErrorType {
    fun message(): String
}

private const val defaultErrorMessage =
    "FireError: a generic error occurred during Firebase operation"
private const val defaultGetItemErrorMessage = "FireError: item has not been found"
private const val defaultInsertItemErrorMessage =
    "FireError: a conflict occurred during the insertion of the item"
private const val defaultDeserializationErrorMessage =
    "FireError: an error occurred deserializing item from Firestore result"
private const val defaultSerializationErrorMessage =
    "FireError: an error occurred Serializing your item to an hash map"
 ...
 * - (2) a DESERIALIZATION_ERROR, if the item has been correctly retrieved
 *   from the cloud but an error occurred during deserialization
 ...
 * - (1) a CONFLICT_ERROR, if the insertion has encountered a conflict with
 *   regard to the db constraints
 * - (2) a SERIALIZATION_ERROR if an error occurred during the
 *   serialization phase into an hashmap (before sending it to the cloud)
...
enum class SaveAndSendNotificationFireError(message: String) : FireErrorType {
    ...
    companion object {
        fun <T> beforeSaveAndSendPush(customMessage: String): Error<T, SaveAndSendNotificationFireError> {
            return Error(NO_SAVE_AND_NO_PUSH_ERROR.also { it.customMessage = customMessage })
        }

        fun <T> beforeSendPush(customMessage: String): Error<T, SaveAndSendNotificationFireError> {
